...
import { getApisResponse, EnquiryList, filterApisResponse, PersonModel, Leads, getEnquiryApisResponse, SourceSubSource } from '../../model/apiresponse';
...
export class EnquiryComponent implements OnInit {
  ...
  enquiryStatus: any = 'Open';
  activeLead: string = '';
  selectSource: string = '';

  paginatedEnquiryList: EnquiryList[] = [];
  totalItems = 0;
  currentPage = 1;
  itemsPerPage = 20;
  ...
  getenquiry(page: number = 1): void {

    const offset = (page - 1) * this.itemsPerPage;
    const { startDateISO, endDateISO } = this.getDateRange();

    const request = {
      FollowenquiryStatus: this.enquiryStatus,
      ...
      Leads: this.activeLead,
      PageSize: this.itemsPerPage.toString(),
      RowStart: offset.toString()
    };

    this.apis.getenquiry(request).subscribe({
      next: (res) => {
        this.apiresponses = res as getEnquiryApisResponse;
        if (this.apiresponses.message && this.apiresponses.message.toLowerCase() === 'success') {
          this.paginatedEnquiryList = this.apiresponses.data as EnquiryList[];
          
          this.leadData = this.apiresponses.leads as Leads[];
          if (this.leadData && this.leadData.length > 0) {
            this.totalItems = this.leadData[0].TotalCountsss ?? 0;
          } else {
            this.totalItems = 0;
          }
  ...
  exportToExcel(): void {
    ...
    this.apis.getDownloadEnquiry(request).subscribe({
      next: (res) => {
        this.apiresponses = res as getEnquiryApisResponse;

        if (this.apiresponses.message && this.apiresponses.message.toLowerCase() === 'success') {
          this.enquiryList = this.apiresponses.data as EnquiryList[];

          if (!this.enquiryList || this.enquiryList.length === 0) {
          ...
          for (let i = 0; i < this.enquiryList.length; i += maxRows) {
            const chunk = this.enquiryList.slice(i, i + maxRows);
            const headers = Object.keys(chunk[0]);

            const worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(chunk, { header: headers });
            const workbook: XLSX.WorkBook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, 'EnquiryData');

            const wbout = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
            const blob = new Blob([wbout], { type: 'application/octet-stream' });

            const fileName = this.enquiryList.length > maxRows
              ? `EnquiryData_Part${part}_${new Date().toISOString().split('T')[0]}.xlsx`
              : `EnquiryData_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  searchValue(input: HTMLInputElement): void {
    ...
    this.apis.getSalesEnqByMobileNo(request).subscribe({
      next: (res) => {
        this.apiresponses = res as getEnquiryApisResponse;
        if (this.apiresponses.message && this.apiresponses.message.toLowerCase() === 'success') {
          this.paginatedEnquiryList = this.apiresponses.data as EnquiryList[];
          this.totalItems = this.paginatedEnquiryList.length;
          this.currentPage = 1;
