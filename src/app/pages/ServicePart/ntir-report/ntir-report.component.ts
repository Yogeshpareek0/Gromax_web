...
import { NTIRResponse } from '../../../model/apiresponse'
import { NTIRReport } from '../../../model/apiresponse'
import { NtirDataResponse } from '../../../model/apiresponse'
import { ToastrService } from 'ngx-toastr';

@Component({
  selector: 'app-ntir-report',
  imports: [CommonModule, FormsModule, PaginationComponent],
  templateUrl: './ntir-report.component.html',
  styleUrl: './ntir-report.component.css'
})

export class NtirReportComponent implements OnInit, OnDestroy {

  pdiData: NTIRReport[] = [];
  apiResponse: NTIRResponse = {
    statusCode: 0, message: '', data: {
      countResponse: {
        totalTractorsCount: 0,
        ntirPendingCount: 0,
        ntirCompletedCount: 0,
        defectFoundCount: 0
      },
      ntirReport: []
    }
  };
  paginatedPDIData: NTIRReport[] = [];
  ...
  private orderedColKeys = [
    ...
    'engineNo', 'runningHours', 'Name', 'section', 'brand', 'remark', 'ntirDate','chasisno'
  ];
  ...
  getPDIReport(page: number = 1): void {
    ...
    this.apis.getNTIRReport(request).subscribe({
      next: (res) => {
        this.apiResponse = res as NTIRResponse;
        if (this.apiResponse.message?.toLowerCase() === 'success') {
          ...
          this.pdiPendingCount = this.apiResponse.data?.countResponse?.ntirPendingCount;
          this.pdiCompletedCount = this.apiResponse.data?.countResponse?.ntirCompletedCount;
          this.defectFoundCount = this.apiResponse.data?.countResponse?.defectFoundCount;

          if (this.apiResponse && this.apiResponse.data?.ntirReport?.length > 0) {
            this.paginatedPDIData = this.apiResponse.data?.ntirReport;
            this.totalItems = this.apiResponse.data?.ntirReport[0].length;

            this.currentPage = page;
          }
          else {
            this.toastr.info(
              'No NTIR Data are available at the moment.',
            );
          ...
          this.toastr.error('Failed fetching NTIR data.', 'Error!');
          /* this.apis.showAlert('error', 'Error!', 'Failed fetching PDI data.');*/
        }
      },
      error: () => {
        this.toastr.error('An error occurred while fetching NTIR data.', 'Error!');
